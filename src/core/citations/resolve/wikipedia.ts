...
export function extractAnchorsFromHtml(
    ...
    if (statement) {
        ...
        const found: string[] = [];
        (scope ?? $("body")).find('sup a[href^="#cite_note"]').each((_, a) => {
    ...
    for (const noteId of noteIds) {
        ...
        if (doi) {
            out.push({
                kind: "doi",
                value: doi.replace(/^https?:\/\/doi\.org\//i, ""),
                raw: doi,
            });
            continue;
        }
        const pmidHref = li
            .find('a[href*="/pubmed/"], a[href*="ncbi.nlm.nih.gov"]')
            .attr("href");
        const pmid = pmidHref?.match(/(\d+)\/?$/)?.[1];
        if (pmid) {
            // biome-ignore lint/style/noNonNullAssertion: A truthy match implies the source href exists.
...
async function resolveIdentifier(
    ...
    try {
        if (idf.kind === "doi")
            return (await d.getWorkByDoi(idf.value, opts))?.node.id ?? null;
        if (idf.kind === "pmid")
            return (await d.getWorkByPmid(idf.value, opts))?.node.id ?? null;
...
export async function resolveWikipedia(
    ...
    if (unique.length === 0)
        throw new Error(`No resolvable references found on ${url}`);

    const { title } = parseWikiUrl(url);
    return {
        claim: statement ?? title.replace(/_/g, " "),
        anchors: unique,
        errors,
    };
}
